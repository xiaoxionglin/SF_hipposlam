...
def main():
    ...
    for row in rows:
        try:
            ...
            missing.append(dict(label=row["label_suffix"], artifact="fields", reason=str(error)))
            continue
        ...
        for layer, mapkey, activitykey, infokey in (
            ...
            information_per_activity = np.divide(
                data[infokey], mean_activity, out=np.full_like(mean_activity, np.nan), where=mean_activity > 0
            )
            summaries.append(
                dict(
                    **identity,
                    layer=layer,
                    observations=int(occ.sum()),
                    visited_bins=int(visited.sum()),
                    total_grid_bins=occ.size,
                    active_units=len(active),
                    silent_units=maps.shape[-1] - len(active),
                    eligible_units=int(eligible.sum()),
                    mono_units=int(mono.sum()),
                    ...
                    unique_peak_bins=unique,
                    peak_entropy=entropy,
                    pairwise_peak_distance_bins=distance,
                ...
                units.append(
                    dict(
                        **identity,
                        layer=layer,
                        unit=unit,
                        active_fraction=float(data[activitykey][unit]),
                        spatial_information=float(data[infokey][unit]),
                        eligible=bool(eligible[unit]),
                        mean_activity=float(mean_activity[unit]),
                        information_per_activity=float(information_per_activity[unit]),
                        mono=bool(mono[unit]),
                        mono_score=float(data[prefix + "_field_mono_score"][unit]),
                    ...
                    components.append(
                        dict(
                            **identity,
                            ...
                            component_count=int(counts[level, unit]),
                        ...
                        plot_rows.append(
                            dict(
                                **identity,
                                ...
                                x_bin=x,
                                y_bin=y,
                                occupancy=int(occ[x, y]),
                                rate=float(maps[x, y, unit]),
                                unit_peak=peak,
                            )
                        )
        maps = data["pre_threshold_rate_maps"]
        allunits = np.arange(maps.shape[-1])
        for unit in (0, 4, 8, 12):
            for x in range(maps.shape[0]):
                for y in range(maps.shape[1]):
                    plot_rows.append(
                        dict(
                            **identity,
                            ...
                            occupancy=int(occ[x, y]),
                            rate=float(maps[x, y, unit]),
                            unit_peak=float(np.nanmax(maps[..., unit])),
                        )
                    )
        ...
        summaries.append(
            dict(
                **identity,
                ...
                observations=int(occ.sum()),
                visited_bins=int(visited.sum()),
                active_only_map_cosine=pairwise_map_cosine(maps, occ),
                unique_peak_bins=unique,
                peak_entropy=entropy,
                pairwise_peak_distance_bins=distance,
                observation_panel=str(data["observation_panel"]),
        ...
        if row["schedule"] == "goal" and not args.fields_only:
            ...
            if not trials.exists():
                missing.append(dict(label=row["label_suffix"], artifact="interventions", reason="Not completed"))
                continue
            ...
            if row["condition"] == "PIC_G_CONTEXT":
                ...
                analyze(
                    trials,
                    path,
                    out / row["label_suffix"] / "context_effective_destination_sensitivity",
                    map_key="rate_maps",
                )
    ...
    manifest.update(
        analysis="PIC navigation diagnostic",
        selected_manifest=str(root / "selected_manifest.tsv"),
        missing=missing,
