...
def main():
    ...
    plt.rcParams.update({'font.family':'DejaVu Sans','font.size':28,'axes.titlesize':28,
        'axes.labelsize':28,'xtick.labelsize':28,'ytick.labelsize':28,'pdf.fonttype':42,
        'svg.fonttype':'none','axes.linewidth':1.2})
    data=pd.read_csv(args.data);outputs=[]
    pre=data[data.layer=='pre_threshold'].rate
    pre_min,pre_max=(float(pre.min()),float(pre.max())) if len(pre) else (0.,1.)
    for (condition,seed,layer),frame in data[data.layer.isin(['raw','pre_threshold'])].groupby(['condition','seed','layer'],sort=False):
        ...
        for ax,unit in zip(axes.flat,(0,4,8,12)):
            ...
            im=ax.imshow(normalized,origin='lower',extent=(1,20,1,20),
                         vmin=0 if layer=='raw' else pre_min,vmax=1 if layer=='raw' else pre_max,
                         cmap='viridis',interpolation='nearest')
            ax.set_title(f'Unit {unit}\nmax {peak:.3g}',pad=10)
            ax.set_xticks([1,10,20]);ax.set_yticks([1,10,20])
            ax.set_xlabel('x / 100');ax.set_ylabel('y / 100')
        ...
        ticks=[0,.5,1] if layer=='raw' else np.linspace(pre_min,pre_max,3)
        bar=fig.colorbar(im,ax=axes,location='bottom',shrink=.85,pad=.04,fraction=.045,ticks=ticks)
        if layer!='raw': bar.ax.set_xticklabels([f'{t:.1f}' for t in ticks])
        ...
        outputs.append(str(stem.with_suffix('.png')));plt.close(fig)
    (args.output/'figure_metadata.json').write_text(json.dumps(dict(
        input=str(args.data.resolve()),font=font,matplotlib=matplotlib.__version__,outputs=outputs,
        selected_units=[0,4,8,12],selection='Fixed before inspecting maps',
        ...
        viewing='PNG intended at about 1000 pixels wide; 28pt source ticks at 150dpi retain approximately 39px at that width.'),indent=2)+'\n')
