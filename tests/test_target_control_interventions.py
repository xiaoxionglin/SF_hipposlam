...
def test_balanced_target_accepts_a_directed_local_successor_row_mask():
    counts = np.zeros((4, 4), dtype=np.int64)
    local_successors = np.array([False, True, False, True])
    assert balanced_target(counts, 0, 5, local_successors) == 1
    counts[0, 1] = 1
    assert balanced_target(counts, 0, 5, local_successors) == 3
