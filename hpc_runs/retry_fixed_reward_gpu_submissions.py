...
def submit_missing(workdir):
    ...
    for row in rows:
        ...
        found = subprocess.run(
            ["squeue", "-u", subprocess.check_output(["whoami"], text=True).strip(), "-h", "-o", "%i %j"],
            text=True,
            capture_output=True,
            check=True,
        ).stdout
        existing = [
            line.split()[0] for line in found.splitlines() if len(line.split()) >= 2 and line.split()[1] == name
        ]
        if len(existing) > 1:
        ...
        if existing:
            ...
            command = [
                ...
                "--parsable",
                "--output",
                str(workdir / "logs" / f"{name}-slurm-%j.out"),
                "--error",
                str(workdir / "logs" / f"{name}-slurm-%j.err"),
                row["sbatch_file"],
            ]
...
def main():
    ...
    if submission.exists():
        ...
        data["jobs"] = [dict(job_id=row["job_id"], status=row["status"], experiment=row["experiment"]) for row in rows]
    ...
    subprocess.run(
        [
            sys.executable,
            "-m",
            "hpc_runs.combine_fixed_reward_manifests",
