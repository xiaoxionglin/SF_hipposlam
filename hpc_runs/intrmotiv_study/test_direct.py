...
def test_source_digest_detects_source_not_outputs(tmp_path):
    source = tmp_path / "sample_factory"
    source.mkdir()
...
def test_queue_records_failure_and_continues_with_later_runs(tmp_path, monkeypatch):
    ...
    success = dict(
        name="success",
        target_frames=10,
        command=[sys.executable, "-c", "print('Total num frames: 10\\nhttps://wandb.ai/e/p/runs/unit_test')"],
    )
    passed = run_queue(dict(common, output_root=str(tmp_path / "pass"), runs=[success]), probe, poll_seconds=0.01)
    assert passed[0]["status"] == "completed"
    assert passed[0]["wandb_urls"] == ["https://wandb.ai/e/p/runs/unit_test"]
    failure = dict(
        name="failure", target_frames=10, command=[sys.executable, "-c", "raise RuntimeError('test failure')"]
    )
    with pytest.raises(RuntimeError, match="run failed"):
        run_queue(dict(common, output_root=str(tmp_path / "fail"), runs=[failure, success]), probe, poll_seconds=0.01)
