...
def make_manifest(study, source, slots):
    ...
    for run in study.expand_runs():
        ...
        command = [
            sys.executable,
            "-m",
            "sf_working_directories.IntrMotiv.dmlab.train_hipposlam",
            *args,
            ...
            for index, owned in list(active.items()):
                ...
                if code is not None:
                    ...
                    ok = (
                        ...
                        and state["frames"] >= spec["target_frames"]
                        and bool(state["wandb_urls"])
                    )
            ...
            if free_slots:
                ...
                for slot in free_slots:
                    ...
                    env = dict(
                        ...
                        OMP_NUM_THREADS="1",
                        MKL_NUM_THREADS="1",
                        PYTHONPATH=manifest["source_root"],
                        INTRMOTIV_SOURCE=manifest["source_root"],
                    )
