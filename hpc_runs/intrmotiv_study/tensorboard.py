    ...
    with executor_type(**options) as executor:
        futures = {
            executor.submit(
                ...
                latest_common or history_output_dir is not None,
            ): index
        ...
        for completed, future in enumerate(as_completed(futures), start=1):
            ...
            if history_output_dir is not None:
                ...
                (history_output_dir / f"{name}.json").write_text(
                    json.dumps({"run_name": name, "sources": sources, "all_selected_events": True}, indent=2) + "\n"
                )
