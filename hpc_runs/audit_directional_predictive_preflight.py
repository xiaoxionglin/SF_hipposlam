"""Audit the 18-cell seed-99 directional/predictive recruitment preflight."""
...
def audit_preflight(study, jobs_tsv: Path, require_submitted: bool = False) -> dict:
    ...
    for row in rows:
        ...
        if require_submitted and (row.get("status") != "submitted" or not row.get("job_id", "").isdigit()):
            raise SpecError(f"preflight run {run_name!r} is not submitted with a numeric job ID")
        observed[run_name] = row
    if set(observed) != set(expected):
        raise SpecError(f"preflight matrix mismatch; missing={sorted(set(expected) - set(observed))}")
    return {
        **study.provenance(),
