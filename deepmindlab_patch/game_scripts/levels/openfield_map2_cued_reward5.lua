-- Five-site number-cued fixed-reward transfer level.
...
-- Five fixed sites. An episode samples one uniformly and exposes its ID (1-5).
-- Site centers: 1=(350, 1950), 2=(250, 1950), 3=(1550, 1250), 4=(650, 1450), 5=(850, 250)
local SOURCE_MAP_1 = [[
*********************
*G     PPPPPPPPPPPPP*
*     *PPPPPPP*PPP*P*
...
function api:init(params)
  api._map = {}
  for site = 1, 5 do
    -- Keep lights and textures identical across the five reward maps.
    make_map.random():seed(1)
    -- Consume the same two map builds as the source level before map 3.
    -- This keeps the source map-3 texture/decal random state for every site.
    make_map.makeMap{mapName = "reward_warmup_1_" .. site,
      mapEntityLayer = SOURCE_MAP_1, useSkybox = true, textureSet = texture_sets.TETRIS}
    make_map.makeMap{mapName = "reward_warmup_2_" .. site,
      mapEntityLayer = SOURCE_MAP_2, useSkybox = true, textureSet = texture_sets.TETRIS}
    api._map[site] = make_map.makeMap{
      mapName = "openfield_reward_site_" .. site,
      mapEntityLayer = TEXT_MAP[site],
      useSkybox = true,
...
function api:nextMap()
  rand_num=random:uniformInt(1, 5)
