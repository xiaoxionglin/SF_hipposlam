        ...
        if packed:
            ...
            if replay_conditions is not None:
                ...
                if self.fixed_task_conditioning:
                    cue = (
                        padded_head[
                            :, :, self.canonical_input_size - self.reward_instruction_count : self.canonical_input_size
                        ]
                        if self.reward_instruction_count
                        else None
                    )
                    goals = self.fixed_task_condition(cue).to(pre)
                    if cue is None:
                        goals = goals.view(1, 1, n).expand_as(pre)
            ...
            for t in range(padded_head.size(0)):
                ...
                if self.fixed_task_conditioning:
                    cue = (
                        padded_head[
                            t, :, self.canonical_input_size - self.reward_instruction_count : self.canonical_input_size
                        ]
                        if self.reward_instruction_count
                        else None
                    )
                    goal = self.fixed_task_condition(cue).to(pre)
                    if cue is None:
                        goal = goal.unsqueeze(0).expand(pre.size(0), -1)
                else:
                    goal = (
                        padded_head[t, :, -n:]
                        if replay
            ...
            if self.fixed_task_conditioning:
                cue = (
                    data[:, self.canonical_input_size - self.reward_instruction_count : self.canonical_input_size]
                    if self.reward_instruction_count
                    else None
                )
                goal = self.fixed_task_condition(cue).to(pre)
