...
from sf_working_directories.IntrMotiv.dmlab.enjoy_telemetry import single_run
from sf_working_directories.IntrMotiv.dmlab.train_hipposlam import (
        ...
        if len(milestones) < cfg.number_epochs_analysis:
            ...
            for i in range(cfg.number_epochs_analysis):
                ...
                if pathlib.Path(milestones[-epoch_i - 1]).stem not in get_folder_names(destination_path):
                    ...
                    log.warn(
                        f"There is already telemetry for this run! Please remove if a re-run is wanted. {cfg.expname} and {milestones[-epoch_i-1]}"
                    )
    else:
