...
class ControllerLearner(DistanceLearnerReward):
    ...
    def save(self):
        # One atomic full restart checkpoint is sufficient. Evaluation history
        # is carried by lightweight milestones rather than replay duplication.
        if getattr(self.cfg, "controller_learning", "ppo") == "ddqn":
            return self._save_impl("checkpoint", "", 1)
        return super().save()

    def save_milestone(self):
        # Milestones are evaluation artifacts, not restart points. Keeping the
        # full physical replay in every milestone multiplies a mature stored-
        # state checkpoint by several GiB without helping evaluation.
        self._evaluation_checkpoint = True
        try:
    ...
    def save_best(self, policy_id, metric, metric_value):
        # Best checkpoints serve model selection and evaluation. Exact training
        # continuation always uses the recent rolling restart checkpoint.
        self._evaluation_checkpoint = True
        try:
    ...
    def _load_state(self, checkpoint_dict, load_progress=True):
        ...
        if load_progress:
            ...
            if not self._restore_controller.get("replay_included", "replay" in self._restore_controller):
                raise RuntimeError("Evaluation checkpoint omits controller replay and cannot resume training")
    ...
    def _get_checkpoint_dict(self):
        ...
        if hasattr(self, "target_snapshot"):
            controller = dict(
                checkpoint_role="evaluation" if getattr(self, "_evaluation_checkpoint", False) else "restart",
                replay_included=not getattr(self, "_evaluation_checkpoint", False),
            ...
            if controller["replay_included"]:
                controller["replay"] = self.replay.state_dict()
    ...
    def _prepare_batch(self, batch):
        ...
        if invalid < size:
            ...
            event_columns = {name: buff[name].detach().cpu().numpy() for name in labels if name in buff}
            keys = [
    ...
    def _update_contextual_anchors(self):
        ...
        for stream, start, end, node in runs:
            if end - start > 1:
                for offset in range(start + 1, end):
                    ...
                    if pair_complete:
                        pair_complete = (
                            valid_cpu[stream, offset : offset + horizon + 1].all()
                            and not done_cpu[stream, offset : offset + horizon].any()
                        )
                    if pair_complete:
        ...
        for stream, start, _end, node in runs:
            ...
            if complete:
                complete = (
                    valid_cpu[stream, start : start + horizon + 1].all()
                    and not done_cpu[stream, start : start + horizon].any()
                )
    ...
    def _ingest(self, batch):
        ...
        observations_cpu = (
            None if stored else {key: cpu_array(value) for key, value in batch["obs"].items() if key != IDENTITY_KEY}
        )
        ...
        final_observations_cpu = (
            None
            if stored
            else {key: cpu_array(value) for key, value in batch["controller_final_obs"].items() if key != IDENTITY_KEY}
        )
