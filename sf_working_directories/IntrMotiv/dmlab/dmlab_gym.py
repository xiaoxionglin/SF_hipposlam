...
from sf_working_directories.IntrMotiv.dmlab.dmlab_level_cache import DmlabLevelCache
from sf_working_directories.IntrMotiv.dmlab.dmlab_utils import string_to_hash_bucket
...
class DmlabGymEnv_custom(gym.Env):
    def __init__(
        ...
        if getattr(self, "with_online_spatial_telemetry", False):
            self.observation_space.spaces["telemetry_pose"] = gym.spaces.Box(
                ...
                high=np.asarray((np.inf, np.inf, np.inf), dtype=np.float32),
                shape=[3],
                dtype=np.float32,
            )
        if self.action_path_integration:
            self.observation_space.spaces["prev_action"] = gym.spaces.Box(
                low=0,
                high=len(self.action_set),
                shape=[1],
                dtype=np.int32,
            )
    ...
    def format_obs_dict(self, env_obs_dict):
        ...
        if position is not None:
            self.last_debug_position = np.asarray(position, dtype=np.float64).copy()
        if rotation is not None:
            self.last_debug_rotation = np.asarray(rotation, dtype=np.float64).copy()
        if self.with_pos_obs:
        ...
        if getattr(self, "with_online_spatial_telemetry", False):
            ...
            env_obs_dict["telemetry_pose"] = np.asarray(
                (self.last_debug_position[0], self.last_debug_position[1], self.last_debug_rotation[1]),
                dtype=np.float32,
            )
        if self.action_path_integration:
            env_obs_dict["prev_action"] = np.asarray([self.previous_action], dtype=np.int32)
    ...
    def _refresh_terminal_debug_position(self) -> bool:
        """Compatibility path that refreshes only terminal position."""
        terminal_observations = self._terminal_debug_observations()
        if terminal_observations is None:
            return False
        position = terminal_observations.get("DEBUG.POS.TRANS")
        if position is None:
            return False
        self.last_debug_position = np.asarray(position, dtype=np.float64).copy()
        return True

    def _refresh_terminal_debug_pose(self) -> bool:
        """Refresh position and yaw telemetry from the terminal state.

        The usual observation path is skipped after DMLab reports that the
        episode ended. Leaving ``last_debug_position`` untouched would label
        the terminal action with the previous state in trajectory telemetry.
        Some DMLab builds do not expose observations after shutdown, so this is
        a best-effort telemetry operation rather than a reason to fail an
        otherwise valid environment step.
        """
        terminal_observations = self._terminal_debug_observations()
        if terminal_observations is None:
            return False
        position = terminal_observations.get("DEBUG.POS.TRANS")
        rotation = terminal_observations.get("DEBUG.POS.ROT")
    ...
    def step(self, action):
        ...
        if (
            ...
            # declared observation and therefore never reaches the policy.
            info["intrmotiv_pose"] = np.asarray(
                [self.last_debug_position[0], self.last_debug_position[1], self.last_debug_rotation[1]],
                dtype=np.float64,
            )
            if terminated:
                info["intrmotiv_terminal_pose_fresh"] = terminal_pose_fresh
        return self.last_observation, reward, terminated, truncated, info
