...
# This is intentionally matched to flat_intrinsic_iterative_baseline.py. The
# persistent controllability graph and target-conditioned hit-distance reward
# are the only architectural additions.
BASE_CLI = (
    ...
    "--wandb_project=SF_IntrMotiv_HRLPersistentTctrl "
    f"--wandb_group={BATCH_NAME} "
    "--wandb_tags hrl persistent_tctrl fast_weights hit_distance nopbt fixed_length "
    "--benchmark=False "
...
def experiment(seed: int, half_life: int, mode_tag: str, iterative: bool) -> Experiment:
    name = f"PHRL_F16_L64_T243_HL{half_life}_{mode_tag}_S{seed}"
    cli = (
        BASE_CLI
        + f"--seed={seed} "
...
RUN_DESCRIPTION = RunDescription(
    BATCH_NAME,
    experiments=[
        experiment(seed, half_life, mode_tag, iterative)
        for seed in SEEDS
