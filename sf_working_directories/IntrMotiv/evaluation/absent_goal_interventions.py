...
def summarize_commands(rows, n_nodes, horizons=HORIZONS):
    ...
    for horizon in horizons:
        ...
        for target in range(n_nodes):
            ...
            for start in sorted({row["start"] for row in rows}):
                ...
                def hit(row):
                    return 0 < row["first_onset_times"][target] <= horizon
    ...
    results["ctc_auc"] = (
        float(np.trapz(values, horizons) / (horizons[-1] - horizons[0]))
        if len(horizons) > 1 and all(v is not None for v in values)
        else None
    )
    results["spatially_qualified_ctc"] = None
    results["spatial_qualification_status"] = (
        "Requires independent canonical-field maps; identity CTC alone is not spatial control."
    )
    return results


def run_absent_goal_interventions(
    cfg,
    env,
    env_info,
    actor,
    ...
    def reset_prefix(start):
        ...
        env = make_env_func_batched(
            evaluation_cfg, env_config=AttrDict(worker_index=0, vector_index=0, env_id=0), render_mode=None
        )
        base_env = env.unwrapped
        ...
        with torch.no_grad():
            for start in range(starts):
                ...
                for command in eligible:
                    ...
                    if (
                        signature.keys() != replay_signature.keys()
                        or any(not torch.equal(value, replay_signature[key]) for key, value in signature.items())
                        or not torch.equal(reference_state, state)
                        or not torch.equal(reference_output, output)
                    ):
                        differences = {
                            key: float((value.float() - replay_signature[key].float()).abs().max())
                            for key, value in signature.items()
                            if key in replay_signature
                        }
                        differences["state"] = float((reference_state - state).abs().max())
                        differences["core_output"] = float((reference_output - output).abs().max())
                        raise RuntimeError(
                            f"Start {start} is not exactly reproducible: {differences}; cannot claim matched intervention"
                        )
                    ...
                    for elapsed in range(1, horizon + 1):
                        ...
                        action = (
                            ...
                            if deterministic
                            else torch.multinomial(probabilities, 1, generator=rng)
                        )
                    ...
                    rows.append(
                        dict(
                            ...
                            complete=complete,
                            first_onset_times=times,
                            first_onset_positions=positions,
    ...
    summary = dict(
        protocol="absent-goal-matched-commands-v1",
        checkpoint=str(checkpoint),
        decisions=decisions,
        decision_cap=decision_cap,
        completed_trials=sum(r["complete"] for r in rows),
        starts_with_insufficient_absent_targets=skipped,
        starts_requested=starts,
        starts_evaluated=len({r["start"] for r in rows}),
