...
def behavior_diagnostics(pose, dg, refractory=8, window=64):
    ...
    for _, group in pose.reset_index(drop=True).groupby(["agent", "num_traj"], sort=False):
        ...
        for period in motifs:
            motifs[period].extend(
                sequence[t - period : t] == sequence[t : t + period] for t in range(period, len(sequence) - period + 1)
            )
    ...
    return dict(
        dominant_onsets=events_total,
        onset_interval_decisions_p10_p50_p90=quantiles(intervals),
        physical_step_distance_p10_p50_p90=quantiles(movement),
        window_straightness_p10_p50_p90=quantiles(efficiency),
        straightness_window_decisions=window,
