...
def bin_indices(points, shape, bounds=(100., 2000., 100., 2000.)):
...
def destination_arrival(row, mask, horizon):
    ...
    result = dict(start_inside=bool(inside[0]), observed_decisions=len(trajectory),
                  hit=False, latency=None, arrival_x=None, arrival_y=None,
                  path_to_arrival=None, straight_line_efficiency=None)
    ...
    result.update(hit=True, latency=index + 1, arrival_x=float(path[-1, 0]),
                  arrival_y=float(path[-1, 1]), path_to_arrival=distance,
...
def evaluate_destinations(rows, masks, horizons):
        ...
        for start in sorted({r['start'] for r in rows}):
            ...
            for target in range(masks.shape[-1]):
                ...
                for trial, outcome in zip(trials, outcomes):
                    records.append(dict(start=start, command=trial['command'], target=target,
                        matched=trial['command'] == target, horizon=horizon, **outcome))
                ...
                latency = lambda o: o['latency'] if o['hit'] else horizon + 1
                contrasts.append(dict(start=start,target=target,horizon=horizon,
                    matched_hit=float(matched['hit']), mismatched_hit=hit_other,
    ...
    for horizon in horizons:
        ...
        means = [{key: float(np.mean([r[key] for r in group if r['target']==t]))
                  for key in ('matched_hit','mismatched_hit','hit_difference','matched_restricted_latency','mismatched_restricted_latency','restricted_latency_difference')}
                 for t in targets]
        summary[str(horizon)] = dict(total_identities=masks.shape[-1], represented_identities=len(targets),
            represented_target_ids=targets, paired_start_target_count=len(group),
            **{key: float(np.mean([r[key] for r in means])) if means else None
               for key in ('matched_hit','mismatched_hit','hit_difference','matched_restricted_latency','mismatched_restricted_latency','restricted_latency_difference')})
        total=masks.shape[-1];missing=total-len(targets)
...
def trajectory_command_effect(rows):
    ...
    endpoints=[];identical=[];divergence=[]
    for start in sorted({r['start'] for r in rows}):
        trials=[r for r in rows if r['start']==start]
        for i,first in enumerate(trials):
            for second in trials[i+1:]:
                ...
                if unequal.any(): divergence.append(int(np.flatnonzero(unequal)[0])+1)
    return dict(paired_command_pairs=len(identical),
...
def analyze(trials, fields, out, map_key='raw_dg_rate_maps'):
    ...
    summary = {k:provenance[k] for k in ('schema','workflow_version','study_sha256','checkpoint','checkpoint_frames','condition','seed') if k in provenance}
    summary.update(fields_sha256=hashlib.sha256(fields.read_bytes()).hexdigest(),
...
def main():
    ...
    parser.add_argument('trials',type=Path);parser.add_argument('fields',type=Path);parser.add_argument('output',type=Path)
