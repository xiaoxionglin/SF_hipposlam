...
def episode(actor, cfg, env, env_info, *, seed, command, target, cell_x, cell_y, horizon):
    ...
    return dict(
        requested_seed=seed,
        engine_seed=actual_seed,
        command=command,
        reward_contact=reward > 0,
        ...
        terminal_x=float(final[0]),
        terminal_y=float(final[1]),
...
def main():
    ...
    cfg, original_env, _, actor, checkpoint, _ = load_policy_env(args.run_dir, args.horizon, False, 0, args.checkpoint)
    original_env.close()
    ...
    summary = {
        ...
        "successes": {
            name: sum(row["reward_contact"] for row in rows if row["condition"] == name)
        ...
        "approach": {
            name: {
                ...
                "mean_minimum_distance": float(
                    np.mean([row["minimum_distance"] for row in rows if row["condition"] == name])
                ),
            }
            for name in ("nominated", "wrong", "shuffled")
        },
