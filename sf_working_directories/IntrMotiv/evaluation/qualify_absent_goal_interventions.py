...
def canonical_components(maps, occupancy):
    ...
    for target in range(maps.shape[-1]):
        ...
        if peak <= 0:
            continue
        labels, count = label((rate >= 0.5 * peak) & (occupancy > 0))
        if count:
...
def qualify(rows, masks, bounds=(100.0, 2000.0, 100.0, 2000.0)):
    qualified = []
...
def main():
    ...
    summary["spatial_qualification_status"] = (
        "Independent raw-DG largest-mass half-peak component, canonical 8-connectivity"
    )
    ...
    summary.update(
        {
            key: provenance[key]
            for key in ("schema", "workflow_version", "study_sha256", "checkpoint", "condition", "seed")
