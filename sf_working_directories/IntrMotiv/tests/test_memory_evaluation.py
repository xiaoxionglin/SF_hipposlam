...
def test_command_control_detects_command_specificity_not_unconditional_visits():
    rows = [
        dict(start=0, command=0, complete=True, eligible_count=2, first_onset_times=[8, 0]),
...
def test_common_panel_records_actions_for_contextual_replay_without_policy_input_changes():
    ...
    from sf_working_directories.IntrMotiv.evaluation.observation_panel import (
        previous_actions_after_step,
        record_observation,
    )
...
def test_panel_replay_preserves_pose_and_episode_resets(tmp_path, monkeypatch):
    ...
    np.savez_compressed(
        path,
        obs_obs=np.ones((3, 1, 2)),
        obs_pos=np.arange(9).reshape(3, 1, 3),
        obs_rot=np.zeros((3, 1, 3)),
        dones=np.array([[False], [True], [False]]),
    )
    pose, activity, _ = panel.replay_observations(
        Actor(), SimpleNamespace(Hippo_n_feature=2, Hippo_R=1, Hippo_L=2), path, "cpu"
    )
