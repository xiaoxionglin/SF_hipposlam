...
def load_case(case, device):
    with np.load(FIXTURE, allow_pickle=False) as saved:
        ...
        values = {name[len(prefix):]: torch.from_numpy(saved[name].copy()).to(device)
                  for name in saved.files if name.startswith(prefix) and name != prefix + 'kwargs'}
    graph = PolicyControllableGraph(values['step0/dg'].size(-1)).to(device)
    graph.load_state_dict({name[len('graph/'):]: value for name, value in values.items()
...
def test_manager_matches_reference(case, device):
    ...
    for step in range(5):
        ...
        result = advance_topological_manager(option, topo, values[prefix+'dg'],
                                             values[prefix+'actions'], graph, **kwargs)
...
def test_manager_has_no_host_scalar_reads_or_dynamic_indices(device):
    ...
    with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU]) as profile:
        advance_topological_manager(values['option'], values['topo'], values['step0/dg'],
                                    values['step0/actions'], graph, **kwargs)
