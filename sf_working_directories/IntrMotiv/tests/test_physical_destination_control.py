...
from sf_working_directories.IntrMotiv.evaluation.physical_destination_control import (
    destination_arrival,
...
def trial(command, path):
    return dict(
        ...
        command=command,
        eligible_count=2,
        eligible_commands=[0, 1],
        start_position=[1050, 1050, 0],
        trajectory=path,
    )
...
def test_incomplete_commands_and_start_inside_do_not_inflate_success():
    ...
    assert summary["1"]["all_identity_matched_hit_bounds"] == [0.5, 1.0]
...
def test_extended_horizons_keep_late_arrivals():
    rows = [
        dict(start=0, command=0, complete=True, eligible_count=2, first_onset_times=[300, 0]),
...
def test_explicit_map_basis_preserves_raw_default_and_labels_sensitivity(tmp_path):
    ...
    trials = tmp_path / "intervention_trials.csv"
    pd.DataFrame(rows).to_csv(trials, index=False)
    trials.with_name("intervention_summary.json").write_text(
        json.dumps(dict(exact_start_verified=True, policy_frozen=True, checkpoint="/tmp/checkpoint.pth", horizons=[1]))
    )
    maps = np.zeros((19, 19, 2))
    maps[0, 0, 0] = 1
    maps[18, 18, 1] = 1
    fields = tmp_path / "fields.npz"
    np.savez(
        fields,
        raw_dg_rate_maps=maps,
        rate_maps=maps[..., ::-1],
        occupancy=np.ones((19, 19)),
        checkpoint="/tmp/checkpoint.pth",
        observation_panel="/tmp/independent.npz",
