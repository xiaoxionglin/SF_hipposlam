...
from sf_working_directories.default.dmlab.custom_decoder import MlpDecoderJit
from sf_working_directories.default.dmlab.custom_learner import BaseDistanceRecorder
...
def single_run(cfg, time_str: str, verbose: bool = False):
    """
    Can be used for generating telemetry for a single run, defined by the loaded config file. (uses load_from_checkpoint)

    :param cfg: normal Config class with cli_args to overwrite
    :type cfg: Config
    :param time_str: A string which defines the foldername of where this run is stored: experiment_dir/telemetry/time_str
    :type time_str: str
